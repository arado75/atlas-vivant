...
function runScenario(name: string, summary: GlobalSummary) {
  ...
  const text = getFocusedSummaryText(summary);

  console.log(`[AtlasMind][Focus][${name}] signal=`, focus);
  console.log(`[AtlasMind][Focus][${name}] text=`, text);
 ...
 * Test manuel v31:
 * - issue dominante
 * - recommandation dominante
 * - motif dominant
 * - fallback status
 */
export function runAtlasMindV31FocusExample() {
  ...
  return {
    ...
    fallbackCase: runScenario("fallback", fallbackSummary)
  };
}
